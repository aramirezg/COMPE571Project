...
def distance(dist):
    ...
    print("function distance is ", dist.value)
    #return TotalDistance
    #return int(distance_result)
...
#distance_result = multiprocessing.Value('i')
...
if __name__ == '__main__':
    try:
        while True:
            ...
            distanceProcess = multiprocessing.Process(target=distance, args=(dist,))
            pictureProcess = multiprocessing.Process(target=picture)
            emailProcess = multiprocessing.Process(target=email)

            distanceProcess.start()
            distanceProcess.join()

            #v = multiprocessing.Value('d',0.0)
            #testing = multiprocessing.Process(target=test, args=(v,))
            #testing.start()
            #testing.join()
            ...
            if dist.value > 2 and dist.value < 40:
                ...
                if snapFlag == 0: #snapFlag = 1 means an intruder is detected for the first time within 2 to 40 cm
                    ...
                    print("\nIntruder detected!", dist.value, "cm")
